function Psvm {
    ...
    # Load modules
    $LoadedModules = Get-Module | Select-Object vmxtoolkit
    if (!$LoadedModules -like "*vmxtoolkit*") { Import-Module -Name vmxtoolkit -Force }

    #Global Varibales
    $script:timeout = 120
    $script:VMS_list = $null
    $script:vm_list_file_path = "C:\Shortcut Scripts\vms.json"
    $script:isWindowClosed = $null
    ...
    $MainFunction = {
        ...
        if ($vm_list_file_path -And (Test-Path $vm_list_file_path -PathType Leaf)) {
